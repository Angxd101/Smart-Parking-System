...
$.get(`${SENSOR_API}`)
    .then(response => {
        const car1Status = response.find(pdata => pdata.id === 2).data;
        $('#car2').text(car1Status);
    });

$.get(`${SENSOR_API}`)
    .then(response => {
        const car1Status = response.find(pdata => pdata.id === 3).data;
        $('#car3').text(car1Status);
    });

$.get(`${SENSOR_API}`)
    .then(response => {
        const car1Status = response.find(pdata => pdata.id === 4).data;
        $('#car4').text(car1Status);
    });
